...
class SpidyQuotesViewStateSpider1(scrapy.Spider):
    ...
    def pagnation(self, response):
        ...
        for facility in facilities[1:]:
            ...
            # name_item = ScrapeazcarecheckprojectItem()
            # #
            # #
            # name_item['name'] = name
            # name_item['address'] = address
            # name_item['facility_type'] = type
            # name_item['city_state'] = city_state
            # yield survey_item

            if name == 'GRACE LIVING CARE':

                with open('grace_file.csv', mode='w') as employee_file:
                    employee_writer = csv.writer(employee_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                    array1 = []
                    array1.append(name)
                    employee_writer.writerow(array)

            if name == 'ACUNA AT MORNING SUN II':

                with open('Acuna_file.csv', mode='w') as employee_file:
                    employee_writer = csv.writer(employee_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                    array = []
                    array.append(name)
                    employee_writer.writerow(array)
